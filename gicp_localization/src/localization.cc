...
void transformCloudData(uint8_t* data, size_t num_points, uint32_t point_step,
  ...
  // does not fit its point — the writes below would smear into the next point
  // and, on the LAST point, past the buffer (heap corruption).
...
gicp_localization::LuminarTimestampRangeNs luminarTimestampRangeFromCloud(
    const sensor_msgs::msg::PointCloud2& msg) {
  gicp_localization::LuminarTimestampRangeNs range;
  ...
  for (size_t i = 0; i < point_count; ++i) {
    ...
    if (!luminarRawTimestampNsFromBytes(
      ...
      return gicp_localization::LuminarTimestampRangeNs{};
    }
...
void shiftCloudTimestamps(uint8_t* data, size_t num_points, uint32_t point_step,
  ...
  // [P3 FIX 2026-07-10] Bounds: widest carrier below is 8 bytes.
  ...
  if (luminar_uint64) {
    ...
    // the same raw uint64 ns bits (see the Luminar decoding notes):
...
void gicp_localization::LocalizationNode::getParams() {
  ...
  // [P3 FIX 2026-07-10] Declared defaults aligned with the shipped yaml.
  ...
  // [REVIEW FIX 2026-07-08 P3] Minimum-support gate (see performLocalization):
  // mean fitness over a handful of inliers can look excellent while the scan
  // barely overlaps the map. <=0 disables either bound.
  ...
  // True residual per-aux clock correction (seconds, ADDED to both header and
  // absolute point times). A header phase delta alone is not evidence of clock
  // error when every Iris is PTP-synchronized; keep zero unless per-point or
  // geometric evidence establishes a residual clock disagreement.
  ...
  this->get_parameter("localization/lidar_concat/future_sweep_wait_timeout", this->concat_future_sweep_wait_s_);
  if (!std::isfinite(this->concat_luminar_time_threshold_) ||
      this->concat_luminar_time_threshold_ <= 0.0) {
    RCLCPP_WARN(this->get_logger(),
                "localization/lidar_concat/luminar_time_threshold=%.3f invalid; using 0.010",
  ...
  this->declare_parameter<double>("localization/debug/jump_trans_m", 30.0);  // [P3 FIX 2026-07-10] yaml-aligned
...
void gicp_localization::LocalizationNode::applyInitialPose(const Eigen::Vector3f& p,
  ...
  }
  {
    ...
    // otherwise unknown (0) -> deskew falls back to prev_scan_stamp.
...
void gicp_localization::LocalizationNode::callbackPointCloud(
  ...
  // [P3 FIX 2026-07-10] Truncated-message guard (parity with the merge path).
  if (pc->point_step == 0 ||
...
bool gicp_localization::LocalizationNode::auxBuffersReadyForPrimary(
  ...
  for (size_t aux_i = 0; aux_i < this->aux_lidars_.size(); ++aux_i) {
    ...
    // A stable but unsupported time schema cannot become usable by waiting for
    // another message. Let mergeAuxClouds take its existing fail-safe path.
    if (candidates.empty()) {
      continue;
    }
...
void gicp_localization::LocalizationNode::waitForFutureAuxSweeps(
...
gicp_localization::LocalizationNode::mergeAuxClouds(
  ...
  for (size_t aux_i = 0; aux_i < this->aux_lidars_.size(); ++aux_i) {
    ...
    // is unsafe on AV-24: the old right sweep can have the nearest header while
    // its rays are one acquisition period early. Generic sensors retain the
    // header-time fallback because they do not expose this absolute carrier.
    ...
    if (usable_time_field) {
      ...
      // absolute carrier ignores dt and applies only the measured clock offset.
    ...
    // not used as a point-clock estimate: synchronized Iris units can retain a
    // stable acquisition phase while their absolute per-point clocks agree.
    ...
    if (++aux.dt_count % 512 == 0) {  // ~every 50 s at 10 Hz
      RCLCPP_INFO(this->get_logger(),
                  ...
                  " — header phase only; use per-point/geometric evidence for clock correction");
    }
  }
...
void gicp_localization::LocalizationNode::deskewPointcloud() {
  ...
  // basePose/base_pose_stamp_/prev_vel are read throughout, and the reinit
  // writers (applyInitialPose, param pose, RTK full seed) run on other
  // threads. Cost: contended only during a reinit. Lock order: seed -> {geo,
  // imu}; performLocalization takes pose -> seed, and no holder of seed ever
  ...
  if (!this->deskew_ || !this->first_imu_received) {
    ...
    if (this->first_imu_received && this->prev_scan_stamp > 0.0) {
      ...
      if (frames.size() == 1 && matrixFinite(frames[0])) {
        ...
        this->T_prior = this->basePoseMatrix();  // [REVIEW FIX 2026-07-08] basePose (scan-chain seed), not current_pose
      }
    } else {
      this->T_prior = this->basePoseMatrix();  // [REVIEW FIX 2026-07-08] basePose (scan-chain seed), not current_pose
    }
  ...
  this->t_prior_stamp_ = timestamps[median_pt_index];  // [P1 FIX 2026-07-10]
...
void gicp_localization::LocalizationNode::performLocalization() {
  ...
  // [REVIEW FIX 2026-07-08 P2] Composition keyed on the per-scan
  // scan_in_world_frame_ flag set by deskewPointcloud(), NOT on deskew_ (see
  // the initial-guess comment above). NOTE: hoisted above the yaw-stiffness
  // block so candidate_pose_valid is declared before its first use there.
  ...
  const bool eigen_projection_wanted =
      ...
      this->gicp_hessian_cond_max_ > 0.0 && final_hessian.allFinite() &&
  ...
  // [REVIEW FIX 2026-07-08 P3] Support of the optimizer solution, consumed by
  // the minimum-support gate below. KNOWN GAP vs GICP_plusplus: nano_gicp has
  // no evaluateFitnessAt(), so when the degeneracy projection / yaw veto
  // modifies the applied pose we cannot re-score fitness/support at
  // final_candidate — the support and fitness gates judge the raw optimizer
  // solution instead (the applied-pose re-scoring is intentionally NOT ported).
  const int support_corr = num_correspondences;
  const double support_ratio = correspondence_ratio;
  ...
  if (effectively_converged && candidate_pose_valid) {
    if (!final_hessian.allFinite()) {
      ...
      // the IMU prior. (Gate on the RAW final hessian: the legacy nano_gicp
      // LM is LEFT-multiplicative (delta * x0), so its Hessian is already in
      // the world-origin tangent on both the world and lidar-frame paths.)
      ...
      if (eigen_projection_wanted && (!degen.valid || degen.fully_degenerate)) {
        ...
        gicp_rejected_jump = true;
               ...
               final_hessian.allFinite() &&  // [P2 FIX 2026-07-10f] +inf condition = rank-deficient, must gate
               hessian_condition > this->gicp_hessian_cond_max_ &&
  ...
  if (gicp_accepted) {
...

    {
      ...
      this->basePose.q = q;
      this->base_pose_stamp_ = this->t_prior_stamp_;  // [P1 FIX 2026-07-10] median-time pose
    }
    ...
    if (!gicp_valid) {
      ...
      if (!this->geo.first_opt_done) {
        ...
        // [REVIEW FIX 2026-07-08 P2/P3] Under geo.mtx — IMU callbacks may be
        // mid-flight in propagateState — and the calibrated IMU biases are
        // PRESERVED: the RTK / stationary calibration paths write state.b.*
        // before the first accepted scan, and unconditionally zeroing them
        // here silently discarded that calibration on non-odom-init setups
        // (use_odom_init=true was immune only because odom init sets
        // first_opt_done, skipping this branch).
        std::lock_guard<std::mutex> geo_lock(this->geo.mtx);
        this->state.p = new_p;
        this->state.q = q;
        this->state.v.lin.w = Eigen::Vector3f::Zero();
    ...
    if (matrixFinite(this->T_prior)) {
      ...
      this->basePose.p = new_p;
      this->basePose.q = q;
      this->base_pose_stamp_ = this->t_prior_stamp_;  // [P1 FIX 2026-07-10]
      {
...
bool gicp_localization::LocalizationNode::maybeSnapPoseToGT(const char* reason) {
  ...
  // [P3 FIX 2026-07-10] Demoted from unconditional INFO (fired every non-accepted scan).
  RCLCPP_DEBUG(this->get_logger(),
  ...
  this->last_accepted_scan_stamp_ = this->scan_stamp.seconds();
  // [P3 FIX 2026-07-10] Post-snap gate/covariance bookkeeping (see GICP++).
  this->last_gicp_pose_ = this->current_pose;
  this->last_gicp_stamp_ = this->scan_stamp;
  this->last_gicp_valid_ = true;
...
void gicp_localization::LocalizationNode::propagateState(const ImuMeas& imu_local) {
  ...
  // distance travelled (speed*time), not by missed-scan count (P3).
  {
    ...
    // holds pose_mutex across the entire GICP solve (tens of ms); taking it
    // here — on EVERY IMU callback, in a MutuallyExclusive group — stalled
    // the whole IMU pipeline for up to gicp_ms each scan (output gaps +
