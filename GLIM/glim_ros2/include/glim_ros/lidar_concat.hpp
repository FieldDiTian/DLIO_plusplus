...
inline sensor_msgs::msg::PointCloud2::ConstSharedPtr merge_clouds(
  ...
  for (size_t aux_i = 0; aux_i < aux_sensors.size(); ++aux_i) {
    ...
    if (primary_luminar_range.valid) {
      ...
      // range (non-UINT8[8] layout, big-endian payload) can never be selected
      // under a Luminar primary — and header matching is NOT a usable
      ...
      if (!aux.buffer.empty() && !any_valid_range) {
        ...
        if (mixed_layout_warns.fetch_add(1) < 3) {
          spdlog::warn(
            "lidar_concat: aux {} carries no decodable absolute UINT8[8] "
            "point-time range (mixed sensor layout or big-endian payload); it "
            "cannot merge under a Luminar primary and header matching is not a "
            "usable fallback (identical schema required) — normalize the "
            "sensor layout upstream (warning capped at 3)",
            aux.topic);
        }
        continue;
      }
