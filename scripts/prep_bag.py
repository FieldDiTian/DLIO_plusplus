...
Typical use (replace the paths with your own; input may be an mcap or sqlite3
rosbag2 directory, or a bare ``.mcap`` file):

    python3 scripts/prep_bag.py \\
      --input  /path/to/raw/run_5/filtered/all \\
      --output /path/to/prepped/run_5_normalized \\
      --dump-dir /path/to/maps/run_5_glim
"""
...
def detect_storage_id(path: Path) -> str:
    """Best-effort rosbag2 storage plugin for an input bag.

    Honors the documented "rosbag2 directory or .mcap file" input contract: a
    bare ``.mcap`` file uses the mcap plugin; a rosbag2 directory is read from
    its ``metadata.yaml`` ``storage_identifier`` (so sqlite3 bags work too),
    falling back to the contained file extension, then to ``mcap``.
    """
    p = Path(path)
    if p.is_file():
    ...
    if meta.is_file():
        m = re.search(r"storage_identifier:\s*['\"]?([A-Za-z0-9_]+)", meta.read_text())
        if m:
            return m.group(1)
    if list(p.glob("*.mcap")):
        return "mcap"
...
def open_bag_reader(path: Path) -> rosbag2_py.SequentialReader:
    ...
    reader.open(
        rosbag2_py.StorageOptions(uri=str(path), storage_id=detect_storage_id(path)),
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    ap.add_argument("--output", required=True, help="normalized rosbag2 output directory (written as mcap storage)")
