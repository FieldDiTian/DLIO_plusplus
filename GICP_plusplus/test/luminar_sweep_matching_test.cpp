...
using gicp_plusplus::LuminarSweepCandidate;
using gicp_plusplus::LuminarTimestampRangeNs;
...
TEST(LuminarSweepMatching, PointTimesOverrideMisleadingHeaderProximity) {
  ...
  const auto selected =
      gicp_plusplus::selectClosestLuminarSweep(primary, candidates);
...
TEST(LuminarSweepMatching, ClockCorrectionAppliesToAbsolutePointRange) {
  ...
  const auto corrected = gicp_plusplus::shiftedRange(uncorrected, -0.010);
...
TEST(LuminarSweepMatching, WatermarkWaitsForFutureAlignedSweep) {
  ...
  EXPECT_FALSE(gicp_plusplus::luminarWatermarkPassed(
      primary, previous, 0.010));
  EXPECT_TRUE(gicp_plusplus::luminarWatermarkPassed(
