...
- **Multi-LiDAR concatenation (`lidar_concat`).** New module in `glim_ros2` (`include/glim_ros/lidar_concat.hpp`) that subscribes to N aux LiDAR topics, selects Iris sweeps by decoded absolute point-time interval (with an offline future-aware queue/watermark), transforms aux points into the primary frame, preserves authoritative absolute point clocks, and emits a single merged cloud. Scan-relative encodings are still rebased onto the primary clock. Includes a validation step that **rolls back the aux-merge append** on a malformed cloud, and the shared **strict merge guard** (`require_all_aux` / `abort_on_merge_failure`, defaults matching GICP — see [Multi-LiDAR merge policy](#multi-lidar-merge-policy)).
