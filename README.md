...
For Luminar Iris, `time_threshold` is not the final authority: both stacks decode the `UINT8[8]` absolute point-time interval and choose the aux sweep whose interval endpoints match the primary within `luminar_time_threshold` (default `0.01 s`). Header deltas remain useful acquisition-phase diagnostics but are not interpreted as PTP clock error. Offline GLIM additionally uses a future-aware primary queue with a `0.15 s` bag-time deadline, so a later-arriving aux can be considered without retaining the last primary until EOF. Live GICP never waits inside its primary callback; it processes the front sweep immediately with whichever coherent aux is already buffered. `aux_match_time_offsets` changes only header matching/tie-breaking; `aux_point_time_offsets` is the separate, explicit residual-clock correction. The old `aux_time_offsets` key is deprecated.
...
Concatenation note: when multiple LiDARs are merged in `lidar_concat`, both stacks now leave Iris's `UINT8[8]` per-point times **unshifted** by header phase (they're already absolute capture times). A nonzero `aux_point_time_offsets` value is applied only when a geometric clock test justifies an actual residual clock correction. Other encodings (FLOAT32/FLOAT64 scan-relative seconds, UINT32 scan-relative ns) still get shifted by `dt = T_aux − T_primary` so they rebase onto the primary's header. See the comment block on `shiftCloudTimestamps` in `gicp_localization/src/localization.cc` and `shift_cloud_timestamps` in `GLIM/glim_ros2/include/glim_ros/lidar_concat.hpp`.
...
- **Multi-LiDAR concatenation (`lidar_concat`).** New module in `glim_ros2` (`include/glim_ros/lidar_concat.hpp`) that subscribes to N aux LiDAR topics, selects Iris sweeps by decoded absolute point-time interval (with an offline future-aware queue bounded in bag time), transforms aux points into the primary frame, preserves authoritative absolute point clocks, and emits a single merged cloud. Scan-relative encodings are still rebased onto the primary clock. Includes a validation step that **rolls back the aux-merge append** on a malformed cloud, and the shared **strict merge guard** (`require_all_aux` / `abort_on_merge_failure`, defaults matching GICP — see [Multi-LiDAR merge policy](#multi-lidar-merge-policy)).
